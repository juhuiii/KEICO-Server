...
class ZbGroupThread 
{
    ...
    async setZbGroupidAssign(){        
        ...
        try{    //On GROUP 
            ...
            {                
                ...
                if( assginGroupId <= 0 )
                {
                    console.error("[ZbGroupThread ] get next group id fail", dev.ZB_ADDR, assginGroupId ); 
                    continue;
                }                                    
                await dbSql.updateOnGroupAssign(dev.ZB_ADDR, assginGroupId);            
            }
            console.debug("[ZbGroupThread ] New Group Assign ID Count : ", deviceRows.length );
        }catch  ( reason )
        {
    ...
    async syncMain() {
        ...
        console.info("[ZbGroupThread ] zigbee group ID sync with smartplug device !!");
    ...
    async zbGroupSyncAndView() {
        ...
        try{
            ...
            console.debug("[ZbGroupThread ]  Send to Mqtt(Add  Group Schedule) : " + message1 );    

            //1초후 확인하기 
            setTimeout( (viw_param) => {
                let message2 = this.makeMqttMessage("GROUP", viw_param)              
                this.sendMqtt(message2);
                console.debug("[ZbGroupThread ]  Send to Mqtt(View Group Schedule) : " + message2 );    
            }, 3000, viw_param);
    ...
    async viewGroupNextCommand() {
        ...
        try{            
            ...
            console.debug("[ZbGroupThread ] Send to Mqtt(View Group Schedule) : " + message );  
    ...
    async makePlugStatSyncchedule(){
        ...
        try{
            ...
            {
                console.info("[ZbGroupThread ] not found new all ON/OFF control commmand !!" );    
                return ;
            }
            ...
            if( ctl_cmd == 0 ) // OFF 제어 
            {
                ...
                console.info("[ZbGroupThread ] Stat Mismatch Count in OffGroup = ", offGroupDevs.length);
            ...
            }else if ( ctl_cmd == 1 ) //ON 제어 
            {
                ...
                console.info("[ZbGroupThread ] Stat Mismatch Count in OnGroup = ", onGroupDevs.length);
    ...
    async syncNextPlugStat() {
        ...
        try{                        
            ...
            console.debug("[ZbGroupThread ] Send to Mqtt(Sync Plug Stat Schedule) : " + message );            
        }catch  ( reason )
        {
            console.error(reason.message);            
        }
    ...
    sendMqtt( message ){    
        mqttAdapter.sendMessage(`${message}`);    
    }  
